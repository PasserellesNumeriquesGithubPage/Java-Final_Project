...
public final class studentDashboard extends javax.swing.JFrame {
    ...
    private void users_update() {
        ...
        ResultSet rs;
        try {
            String select_query = "SELECT usersId FROM student_info WHERE studentId = ?";
            ps = StudentRecord.getConnection().prepareStatement(select_query);
            ps.setString(1, id.getText());
            rs = ps.executeQuery();
            if (rs.next()) {
                String usersID = rs.getString(1);
                int u;
                System.out.println("My Id: " + usersID);
                String query = "SELECT tblcourses.courseCode, tblcourses.courseDescription, tblcourses.courseUnit, student_info.MG, student_info.FG FROM student_info"
                        + " INNER JOIN tblcourses ON student_info.courseId = tblcourses.courseId "
                        + "WHERE student_info.usersId = ? ";
